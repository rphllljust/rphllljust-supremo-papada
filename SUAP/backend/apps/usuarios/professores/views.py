...
def professores_list(request):
    professores = Usuario.objects.filter(tipo=PerfilUsuario.PROFESSOR).order_by("first_name", "last_name", "username")
...
def professores_update(request, pk):
    professor = get_object_or_404(Usuario, pk=pk, tipo=PerfilUsuario.PROFESSOR)
